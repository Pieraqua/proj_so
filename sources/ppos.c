...
void ppos_init()
{
   ...
   filaTarefas = NULL;
    ...
    main->id = maiorId + 1;                                       // define a id da task
    main->status = 0;                                             // Define o status da task a ser criada como Pronta
    getcontext(&(main->context));                                 // Copia o contexto atual e copia para contextoTask
    main->context.uc_stack.ss_flags = 0;                          // Deixa como zero a máscara das flags
    main->context.uc_link = 0;                                    // Quando a task terminar ela não tem para onde voltar, então deixa como 0
    main->preemptable = 0; // Define a variavel preempable da task do elemento de fila como 0, ou seja não preemptável

    queue_append((&filaTarefas), main);
